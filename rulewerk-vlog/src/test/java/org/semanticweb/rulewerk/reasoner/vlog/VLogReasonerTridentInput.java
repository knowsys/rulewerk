...
public class VLogReasonerTridentInput {
	...
	@Ignore
	// Test fails, see https://github.com/karmaresearch/vlog/issues/55
	@Test
	public void testLoadTernaryFactsFromSingleRdfDataSource() throws IOException {
		final DataSource fileDataSource = new TridentDataSource(
				FileDataSourceTestUtils.INPUT_FOLDER + "tridentTernaryFacts");
	...
	@Ignore
	// Test fails, see https://github.com/karmaresearch/vlog/issues/56
	@Test(expected = IOException.class)
	public void tridentDbDoesNotExist_fails() throws IOException {
		final File nonexistingFile = new File("nonexisting");
		assertFalse(nonexistingFile.exists());
		final DataSource dataSource = new TridentDataSource(nonexistingFile.getName());
		final KnowledgeBase kb = new KnowledgeBase();
		kb.addStatement(new DataSourceDeclarationImpl(ternaryPredicate, dataSource));
